...
follows a Taylor-like monetary policy rule, which is a function of the inflation rate and determines the change in money supply. 
The Taylor-like monetary policy rule is given by:
\begin{align}
    \frac{M_t}{M_{t-1}} = \biggl[ \frac{1+\pi_t}{1+\bar{\pi}} \biggr]^{-\theta_\pi}
\end{align}

\textbf{Explain here the equation and how it affects the economy.}
...
As our scenario requires that the Central Bank adopts a money supply target aiming to achieve price stability, we adjust the benchmark model to a monetary policy rule with the following specification: \textbf{I wrote the same equation in section 3b? Also I dont get how the CB can set $M_t$ using $\pi_t$}
...
\textbf{Put here the IRFs for different Thetas}
\begin{figure}{H}
    \caption{Impulse response functions of observable variables}\label{fig:obs_var_taylor_2}
    \centering
    \includegraphics[width=0.7\textwidth]{../matlab_base/output/irf_gdp_taylor.png}
\end{figure}
% Content here.
